...
class Node:
	...
	def __init__(self):
		...
		self.nid = None
	@classmethod
	def populated(cls, matches, fnode, symbol, string):
...
if len(sys.argv) < 5:
	print "Usage: -p <patterns.txt> -t <text>"
	sys.exit(0)

if sys.argv[1] == "-t" and sys.argv[3] == "-p":
	text = sys.argv[2]
	patternfile = sys.argv[4]
elif sys.argv[3] == "-t" and sys.argv[1] == "-p":
	text = sys.argv[4]
...
for p in patterns:
	...
	for c in p:
		...
		if child != None:
			...
			newc = Node.populated([], startnode, c, processedStr)
			# Wir müssen den neuen Knoten in den Baum einfügen, nähmlich
...
for c in text:
	...
	if nextn != None:
		# Es wurde ein Knoten erreicht, der nicht der Wurzelknoten ist(Denn ist
                # er durch ein Fail-Transition an den Startknoten gekommen und hat dann
                # keinen Nachfolger gefunden, ist nextn = None).
		# Das kann bedeuten, dass Matches gefunden wurden. Gebe alle Matches
