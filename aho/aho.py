...
#
# This is a modified Aho-Corasick algorithm. It users the tree
# and the failnodes for matching words.
#
# The code for the original Aho-Corasick is contributed from
# Johann Beleites
...
#dessen, deshalb, dennoch - stimmen da die Ratings bzw der Pfad der gewählt wird?
def followHighRatedPath(curn):
    nextn = curn
    matches = []

    while nextn != None:
        matches.append([nextn.rating, nextn.matches])
        oldn = nextn
        nextn = nextn.getChildWithMaxRating()

    return matches

def addLenghtBonus(matchesArray):
    lenBounsFaktor = 25
    
    #delete empty Matches
    matchesArray = [x for x in matchesArray if x[1]] 
    
    #only keep longest Matching if multiple
    for entry in matchesArray:
        entry[1] = max(entry[1], key=len)
    
    #add lenghtbonus
    for entry in matchesArray:
        entry[0] = int(entry[0]) + (len(entry[1] ) * len(entry[1]) * lenBounsFaktor)
...
if __name__ == "__main__":
    ...
    while 1:
        ...
        matchesArray = followHighRatedPath(state)
        matchesArray = addLenghtBonus(matchesArray)
        highestRatingMatch = sorted(matchesArray, key=itemgetter(0))[-1] 

        print(matchesArray)
        print("Vorschlag: ",highestRatingMatch)
        ctr += 1
